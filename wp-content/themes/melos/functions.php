...
// Определение функции wpcourses_breadcrumb, которая принимает в качестве аргумента разделитель ($sep)
// по умолчанию ' > '. Функция использована для отображения навигационной цепочки (breadcrumbs) в WordPress.
function wpcourses_breadcrumb( $sep = ' > ' ) {
	// Глобализация $post, что означает, что мы используем глобальную переменную $post внутри функции.
	global $post;

	// Объявление переменной $out и инициализация ее пустой строкой. Эта переменная будет использоваться для
    	// хранения HTML-кода breadcrumbs.
	$out = '';

	// Начало формирования HTML-кода breadcrumbs. Создание div с классом wpcourses-breadcrumbs.
	$out .= '<div class="wpcourses-breadcrumbs">';

	// Добавление ссылки на главную страницу.
	$out .= '<a href="' . home_url( '/' ) . '">Главная</a>';

	// Добавление разделителя после ссылки на главную страницу.
	$out .= '<span class="wpcourses-breadcrumbs-sep">' . $sep . '</span>';

	// Проверка, отображается ли запись. Если да, то нам нужно включить в breadcrumbs категорию этой записи.
	if ( is_single() ) {
		// Получение категорий текущей записи.
		$terms = get_the_terms( $post, 'category' );

		// Проверка, есть ли у записи категории. Если да, то добавить первую категорию в breadcrumbs.
	...
	// Если мы находимся на странице записи (любого типа, включая страницы), добавить заголовок этой страницы в breadcrumbs.
	if ( is_singular() ) {
		$out .= '<span class="wpcourses-breadcrumbs-last">' . get_the_title() . '</span>';
	}

	// Если мы находимся на странице категории, добавить название этой категории в breadcrumbs.
	if ( is_category() ) {
		$out .= '<span class="wpcourses-breadcrumbs-last">' . single_cat_title('', false) . '</span>';
	}

	// Если совершается поиск, добавить поисковый запрос в breadcrumbs.
	if ( is_search() ) {
	...
	// Возврат сформированного HTML-кода breadcrumbs.
	return $out;
}
...
// Определение функции my_taxonomies_news, которая будет использоваться для регистрации таксономий
// (классификаций постов) в WordPress. В данном случае мы регистрируем две таксономии: 'news_category' и 'news_tag'.
function my_taxonomies_news() {
   // Определение массива $labels, который содержит набор пользовательских текстовых меток
   // для таксономии 'news_category'. Эти метки будут использоваться в различных частях административного интерфейса WordPress.
   $labels = array(
      // Нужен для названия меню в админке.
      'name'              => _x( 'Категории новостей', 'taxonomy general name', 'melos' ),
	// Используется там, где необходимо использовать единственное число.
      'singular_name'     => _x( 'Категория новостей', 'taxonomy singular name', 'melos' ),
	// Текст кнопки для поиска категории.
      'search_items'      => __( 'Найти категорию новостей', 'melos' ),
	// Текст для ссылки показывающей все элементы таксономии.
      'all_items'         => __( 'Все категории новостей', 'melos' ),
	// Текст для родительской таксономии.
      'parent_item'       => __( 'Родительская категория новостей', 'melos' ),
	// Текст для родительской таксономии с использованием символа ":".
      'parent_item_colon' => __( 'Родительская категория новостей:', 'melos' ),
	// Текст для редактирования элементов таксономии.
      'edit_item'         => __( 'Редактировать категорию новостей', 'melos' ),
	// Текст для обновления элемента таксономии.
      'update_item'       => __( 'Обновить категорию новостей', 'melos' ),
	// Текст кнопки для добавления нового элемента таксономии.
      'add_new_item'      => __( 'Добавить новую категорию новостей', 'melos' ),
	// Текст для нового имени элемента таксономии.
      'new_item_name'     => __( 'Новая категория новостей', 'melos' ),
	// Текст названия в меню.
      'menu_name'         => __( 'Категории новостей', 'melos' ),
   );

   // Определение аргументов, которые будут использоваться при регистрации таксономии 'news_category' в WordPress.
   $args = array(
	// Установка пользовательских меток для данной таксономии.
      'labels' => __($labels, 'melos'),
	// Установка для данной таксономии иерархической структуры (как у категорий).
      'hierarchical' => true,
	// Делаем таксономию публично доступной.
      'public' => true,
	// Включаем отображение колонки таксономии в таблице записей в админпанели.
      'show_admin_column' => true
   );
	
   // Регистрация таксономии 'news_category' для типа записей 'news' с заданными в $args аргументами.
   register_taxonomy( 'news_category', array('news'), $args );
	
	// Ниже происходит аналогичный процесс для таксономии 'news_tag', но в этом случае таксономия не иерархическая,
    	// какая часто бывает для меток (tags).
